...
def inference(loader, model, device, view, data_size):
    ...
    h_vector = np.array(h_vector).reshape(data_size, 128)
    total_pred = np.argmax(np.array(soft_vector), axis=1)
    # for v in range(view):
    #     Hs[v] = np.array(Hs[v])
    #     Zs[v] = np.array(Zs[v])
    #     pred_vectors[v] = np.array(pred_vectors[v])
    return total_pred, labels_vector, h_vector, total_pred
...
def valid(model, device, dataset, view, data_size, class_num, eval_h=False):
    ...
    total_pred, labels_vector, H, Q = inference(test_loader, model, device, view, data_size)
...
def eva(model, device, dataset, view, data_size, class_num, eval_h=False):
    test_loader = DataLoader(
        dataset,
        batch_size=data_size,
        shuffle=False,
